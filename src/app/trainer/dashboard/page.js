...
export default async function TrainerDashboardPage() {
  ...
  const [allBookings, clients, profile, allReviews, ratingSummary] =
    await Promise.all([
  ...
  const estimatedEarnings =
    completedSessions * (Number(profile?.price) || 0);
  ...
  const stats = [
    {
      ...
      change: `${clients.length} unique client${
        clients.length !== 1 ? "s" : ""
      }`,
    },
    {
      ...
      change: upcomingCount > 0 ? "Pending or confirmed" : "None scheduled",
    },
    {
      ...
      change: `${completedSessions} completed session${
    ...
    },
    {
      ...
      change: `${ratingSummary.totalReviews} review${
